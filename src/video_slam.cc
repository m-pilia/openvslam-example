        ...
        )
{
        ...
        while (is_not_end) {
            ...
            if (!frame.empty() && (num_frame % frame_skip == 0)) {
                if (cfg->camera_->setup_type_ == openvslam::camera::setup_type_t::Monocular) {
                    ...
                    const auto frame_left = frame.colRange(0, frame.cols / 2);
                    const auto frame_right  = frame.colRange(frame.cols / 2, frame.cols);
                    SLAM.feed_stereo_frame(frame_left, frame_right, timestamp, mask);
                }
                else {
    ...
    if (!localization && !map_db_path.empty()) {
...
int main(int argc, char* argv[]) {
    ...
    if (localization->is_set() && !map_db_path->is_set()) {
